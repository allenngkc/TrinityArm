...
void servoarm(direction) {
  Servo motorarm;
  int pos=0;    // variable to store the servo position
  int move1;
  int move2;
  void setup() {
    motorarm.attach(9);  // attaches the servo on pin 9 to the servo object
    Serial.begin(9600);
  }
  void loop(direction) {
    if (direction == "left") {
      move1 = 20;
      for (pos = 0; pos <= move1; pos+=1) {
        if (pos < move1) {
          pos += 1;
        }
        delay(5);
        motorarm.write(pos);
      } //receive xpos from glasses
    }
...
void servowrist(direction) {
  Servo motorwrist;
  int pos=0;    // variable to store the servo position
  int move1;
  int move2;
  void setup() {
    motorwrist.attach(8);  // attaches the servo on pin 9 to the servo object
    Serial.begin(9600);
  }
void loop(direction) {
  if (direction == "up") {
